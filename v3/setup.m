...
RvpW_old=zeros(2,nsteps+1); RvpW_new=zeros(2,nsteps+1);
V_wp=zeros(2, nsteps);
...
for i=1:nel %recorre los puntos de control
    ...
    A(i,nel+1)=normalsB(:,i)'*Biot_Savart(rcpB(1, i), rcpB(2, i),rcpB(1, j)+long_el/2, rcpB(2, j), 1, coff);
end
A(nel+1,:)=1;
...
nelw=0;
RvpW_old(:, 1)=RnpN(:, nnp)+ [long_el/4*cos(theta);-long_el/4*sin(theta)];
...
nelw=nelw+1;
 RvpW_old(:, :)=RvpW_new(:,:);
    ...
    figure(10)
    %plot(RvpW_old(1, 1:nelw),RvpW_old(2, 1:nelw),'*r')
...
%% simulacion con movimiento de la placa 
else
