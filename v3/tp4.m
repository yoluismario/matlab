...
[t,xi]=ode45(@(t,x) osciladorsimple(t,x,A),[0 10], [5,10]) ;
X(1:length(xi),1:2,length(IC)+1)=xi;
[t,xi]=ode45(@(t,x) osciladorsimple(t,x,A),[0 10], [5,-10]) ;
X(1:length(xi),1:2,length(IC)+2)=xi;
xl=-5:.01:5;
lambda1=Lambda(1,1)*xl;
lambda2=Lambda(2,2)*xl;
...
for i=2:length(IC)
    aux=X(:,:,i);
    fin=find(aux== 0);
    if (length(fin)==1)||(isempty(fin))
    final=size(X,1);
    else
    final= min(size(X,1), fin(1)-1);
    end
    plot(X(1:final,1, i), X(1:final,2,i))
...
%    plot(X(1:final,1, length(IC)+1), X(1:final,2,length(IC)+1))
%        xq=X(5:5:size(X,1)-1,1, length(IC)+1); yq=X(5:5:size(X,1)-1,2,length(IC)+1);
%        uq = zeros(size(xq));
% vq = zeros(size(xq));
%     for j=1:numel(xq)
%         dx = osciladorsimple(t,[xq(j); yq(j)], A);
...
%     quiver(xq,yq,uq,vq,.2,'r')%,'Autoscale','off');
%     
%        plot(X(1:final,1, length(IC)+2), X(1:final,2,length(IC)+2))
%        xq=X(5:5:size(X,1)-1,1, length(IC)+2); yq=X(5:5:size(X,1)-1,2,length(IC)+2);
%        uq = zeros(size(xq));
% vq = zeros(size(xq));
%     for j=1:numel(xq)
%         dx = osciladorsimple(t,[xq(j); yq(j)], A);
...
%     quiver(xq,yq,uq,vq,.2,'r')%,'Autoscale','off');
    ...
    plot(xl, lambda1,'g',xl, lambda2,'g')
 axis equal
