...
xlim([0 2])
title('Solucion de la ecuacion de Blasius', 'FontSize', 14);
xlabel('f, f'' y f''''', 'FontSize', 20);
ylabel('\eta', 'FontSize', 20);
grid on
Legend1 = {'f(\eta)', 'f''(\eta)', 'f''''(\eta)'};
legend(Legend1, 'FontSize', 14);

%% Perfiles de Velocidad y distribución de espesor de CL
figure(2)
hold all
for i = 1:length(x)
...
Legend2 = {'\delta (x)', 'Perfil de velocidad en x = 1', 'Perfil de velocidad en x = 4', 'Perfil de velocidad en x = 5'};
title('Perfiles de Velocidad a x = 1, 4 y 5; espesor de CL a lo largo de la placa plana', 'FontSize', 14);
...
title(' tensiones de corte en la pared a lo largo de la placa plana', 'FontSize', 14);
xlabel('x', 'FontSize', 20);
...
title(' tensiones de corte como función de \eta ', 'FontSize', 14);
xlabel('\eta', 'FontSize', 20);
ylabel('\tau', 'FontSize', 20);
axis tight
grid on
Legend3 = {'\tau(\eta) en x = 1', '\tau(\eta) en x = 4', '\tau(\eta) en x = 5'};
...
% % Accumulative Drag Force along the flat plate
% title(' Fuerza de resistencia acumulado a lo largo de la placa plana', 'FontSize', 14);
% xlabel('x', 'FontSize', 20);
% ylabel('Fuerza de resistencia (D)', 'FontSize', 20);
% axis tight
% grid on
